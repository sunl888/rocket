...
public final class ExtensionLoader<T> {
    private static final String EXTENSION_LOAD_DIRECTORY = "META-INF/rocket/";
    ...
    public T getExtension(final String name) {
        if (StringUtils.isBlank(name)) {
            throw new NullPointerException("get extension name is null");
        }
    ...
    public List<T> getExtensions() {
        ...
        classEntities.forEach(v -> {
            T extension = this.getExtension(v.getName());
    ...
    private void createExtension(final String name, final Holder<Object> holder) {
        ...
        Class<?> aClass = classEntity.getClazz();
        Object o = extensionInstances.get(aClass);
        if (Objects.isNull(o)) {
            o = createExtension(classEntity);
            if (classEntity.isSingleton()) {
                extensionInstances.putIfAbsent(aClass, o);
                o = extensionInstances.get(aClass);
            }
        }
    ...
    private void loadDirectory(final Map<String, ClassEntity> classes) {
        String fileName = EXTENSION_LOAD_DIRECTORY + clazz.getName();
        try {
    ...
    private void loadClass(final Map<String, ClassEntity> classes,
        ...
        if (!subClass.isAnnotationPresent(Adaptive.class)) {
            throw new IllegalStateException("load extension resources error," + subClass + " without @" + Adaptive.class + " annotation");
        }
        ClassEntity oldClassEntity = classes.get(name);
        if (Objects.isNull(oldClassEntity)) {
            Adaptive adaptiveAnnotation = subClass.getAnnotation(Adaptive.class);
            ClassEntity classEntity = new ClassEntity(name, adaptiveAnnotation.order(), subClass, adaptiveAnnotation.isSingleton());
